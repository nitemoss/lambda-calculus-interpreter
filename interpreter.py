...
# PART 9 — FILE LOADER
# ---------------------------------------------------------------------------
# A .lc file contains lines of the form:
#   name = term          (definition, stored for reuse)
#   term                 (expression to evaluate and print)
#   -- comment           (ignored)
...
def run_file(path: str, env: Environment, trace: bool = False):
    ...
    for lineno, raw in enumerate(lines, 1):
        ...
        if not line or line.startswith("--"):
            continue
        try:
            # Definition:  name = term  (name must be a plain identifier)
            parts = line.split("=", 1)
            lhs = parts[0].strip()
            if (len(parts) == 2
...
# ---------------------------------------------------------------------------
# PART 10 — REPL (Read-Eval-Print Loop)
# ---------------------------------------------------------------------------
